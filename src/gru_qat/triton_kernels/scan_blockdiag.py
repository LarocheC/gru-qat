"""Blockdiag (block-diagonal) hidden weights for the multi-step GRU scan.

Tier-2 work: structured-hidden-side variant of ``gru_scan_persistent``.
The hidden weight ``Wh`` is parameterized as three Blockdiag factors (one
...
def extract_blockdiag_factors(cell: nn.Module) -> tuple[torch.Tensor, torch.Tensor]:
    """Pull the three hidden-side Blockdiag weights out of a tier-1 cell.

    Args:
        cell: a ``GRUCellQuant`` whose ``structure_hidden`` was a Blockdiag
...
def gru_scan_blockdiag_forward_pytorch(
    gi: torch.Tensor,
    h0: torch.Tensor,
    ...
    *,
    ...
    assert n_gates == 3
    assert out_blksz == in_blksz, "square Blockdiag only"
...
def gru_scan_blockdiag_fwd_kernel(  # type: ignore[no-untyped-def]
    gi_ptr,            # [T, B, 3H], fp32
    h0_ptr,            # [B, H], fp32
...
def gru_scan_blockdiag_forward_triton(
    gi: torch.Tensor,
    h0: torch.Tensor,
    Wh_struct: torch.Tensor,
    bh_cat: torch.Tensor,
    *,
    ...
    """Triton forward for the Blockdiag hidden-side scan."""
    ...
    gru_scan_blockdiag_fwd_kernel[grid](
...
@triton.jit  # type: ignore[untyped-decorator]
def gru_scan_blockdiag_bwd_kernel(  # type: ignore[no-untyped-def]
...
def gru_scan_blockdiag_backward_triton(
    gi: torch.Tensor,
    h0: torch.Tensor,
    Wh_struct: torch.Tensor,
    bh_cat: torch.Tensor,
    ...
    *,
    ...
    """Triton backward for the Blockdiag hidden-side scan."""
    T, B, three_H = gi.shape
    H = three_H // 3
    n_gates, nblocks, out_blksz, in_blksz = Wh_struct.shape
    if n_gates != 3:
    ...
    grid = (n_pid_b, nblocks)
    gru_scan_blockdiag_bwd_kernel[grid](
        gi, h0, Wh_struct, bh_cat, out,
        dout,
        dgi, dh0,
...
class GRUScanBlockdiagFunction(torch.autograd.Function):
    """autograd wrapper around the Blockdiag persistent kernels.
    ...
    def forward(
        ...
        out = gru_scan_blockdiag_forward_triton(
    ...
    def backward(
        ...
        grads = gru_scan_blockdiag_backward_triton(
...
def gru_scan_blockdiag(
    ...
    *,
    ...
    """Public API: differentiable Blockdiag-hidden-side GRU scan.
    ...
    Use ``extract_blockdiag_factors(cell)`` to pull factors out of a
    tier-1 structured GRUCellQuant.
    """
    return cast(
        torch.Tensor,
        GRUScanBlockdiagFunction.apply(  # type: ignore[no-untyped-call]
            gi, h0, Wh_struct, bh_cat, h_in_quant, h_out_quant
        ),
    )


def gru_scan_blockdiag_backward_pytorch(
    gi: torch.Tensor,
    h0: torch.Tensor,
    Wh_struct: torch.Tensor,
    bh_cat: torch.Tensor,
